...
<div style="background-color: #eee; padding: 10px 10px 2px 10px">
  <p>
    <b style="color:red">⚠️ 重要提示: </b>近期弱点升级，基础包版本均要求 <b><i>Nodejs >= 20</i></b>，如果有本地二次开发需要，请先做好Nodejs版本升级，再进行开发以及构建。
  </p>
</div>
...
所有主题色彩、间距、z-index 均通过 CSS 自定义变量统一管理，定义在 `src/styles/theme-variables.css` 的 `:root` 块中。所有组件均引用变量而非硬编码数值，修改主题只需改变量即可全局生效。

命名規范：`--gantt-{category}-{variant}`。已有分组：`bg`（背景色）、`text`（文字色）、`border`（边框色）、`z`（层叠阶层）。

#### z-index 分层规范

参考 Element Plus 设计规范，将 z-index 分为**视口级**和**局部层**两大体系，全面替代硬编码数值。

| 变量 | 值 | 层级范围 | 所属层叠上下文 | 用途 |
|---|:---:|---|---|---|
| `--gantt-z-overlay` | 9999 | 视口级 | 视口（position:fixed，逃脱所有 SC） | `position:fixed` tooltip、右键菜单、全屏模式，需高于宿主页面 modal |
| `--gantt-z-toggle` | 1000 | 容器级 | 宿主/文档初始 SC | 任务列表折叠按鈕，防被 sticky header 遮挡 |
| `--gantt-z-sticky` | 30 | 容器级 | 宿主/文档初始 SC | 聚焦关闭按鈕、年视图今日标记线等浮层装饰 |
| `--gantt-z-canvas-hl` | 20 | 容器级 | 宿主/文档初始 SC | GanttLinks canvas 高亮状态（需 > --gantt-z-row:11） |
| `--gantt-z-canvas` | 12 | 容器级 | 宿主/文档初始 SC | GanttLinks canvas 普通状态 |
| `--gantt-z-row` | 11 | 容器级 | task-bar-container SC | `.task-row` / `.resource-row`（建立局部层叠上下文） |
| `--gantt-z-container` | 10 | 容器级 | 宿主/文档初始 SC | `.task-bar-container`（建立局部层叠上下文） |
| `--gantt-z-bar-drag` | 40 | row 内局部 | task-row SC | task-bar / milestone 拖拽中（最高交互层） |
| `--gantt-z-avatar` | 35 | row 内局部 | task-row SC | 头像堆叠层 |
| `--gantt-z-actual-hl-pri` | 32 | row 内局部 | task-row SC | actual-bar primary-highlight |
| `--gantt-z-bar-hl-pri` | 30 | row 内局部 | task-row SC | task-bar primary-highlight |
| `--gantt-z-actual-hl` | 28 | row 内局部 | task-row SC | actual-bar highlighted |
| `--gantt-z-bar-hl` | 26 | row 内局部 | task-row SC | task-bar highlighted |
| `--gantt-z-milestone-sticky` | 24 | row 内局部 | task-row SC | milestone sticky 停靠模式（停靠到左边缘） |
| `--gantt-z-conflict` | 22 | row 内局部 | task-row SC | GanttConflicts canvas |
| `--gantt-z-milestone` | 20 | row 内局部 | task-row SC | milestone 普通状态 |
| `--gantt-z-bar-bubble` | 18 | row 内局部 | task-row SC | task-bar has-bubble 气泡指示 |
| `--gantt-z-bar-hover` | 15 | row 内局部 | task-row SC | task-bar hover / has-actual:hover |
| `--gantt-z-bar-actual` | 12 | row 内局部 | task-row SC | actual-bar 实际进度条 |
| `--gantt-z-bar` | 10 | row 内局部 | task-row SC | task-bar 基础层 |

> **层叠上下文隔离模型（三级）：**
> - `.gantt-root` 无 `position` / `z-index`，**不建立层叠上下文**，整体参与宿主文档的默认层叠排序。
> - `.task-bar-container`（position:absolute + z-index:10）建立 SC → `.task-row`（z-index:11）仅在此上下文内排序。
> - `.task-row`（position:absolute + z-index:11）建立 SC → TaskBar / Milestone 等所有 row 内元素仅在行内排序，与组件外完全隔离。
>
> **对宿主应用的影响：**
> 1. `position:fixed` 浮层（`--gantt-z-overlay: 9999`，含 tooltip、右键菜单、全屏）逃脱所有层叠上下文，会覆盖宿主中 z-index < 9999 的元素。若宿主 modal 层级 ≥ 9999，可按需覆盖变量：`:root { --gantt-z-overlay: 500; }`。
> 2. 其余定位元素（z-index 10 ~ 1000）受限于 DOM 布局盒，实践中与宿主内容不冲突；如需完全隔离，可在宿主侧 gantt 容器加 `isolation: isolate`。

| 元素 | 分层规范 | 原因 |
|---|---|---|
| `position:fixed` 加载遗罩 | 10000 （保留硬编码） | JS 动态创建的临时 DOM，生命周期极短 |
| `.task-bar.dimmed` | 1 （保留硬编码） | 刻意压到最低，不参与常规层级竞争 |
| `::before/::after` 内部元素 | 1/-1 （保留硬编码） | 不影响外部层叠上下文 |
